...
(function(gdjs2) {
  class PlatformerObjectRuntimeBehavior extends gdjs2.RuntimeBehavior {
    ...
    doStepPreEvents(runtimeScene) {
      ...
      this._releasePlatformKey || (this._releasePlatformKey = !this._ignoreDefaultControls && inputManager.isKeyPressed(DOWNKEY));
      ...
      this._releasePlatformKey = false;
      this._jumpKey = false;
      this._hasReallyMoved = Math.abs(object.getX() - oldX) >= 1 || Math.abs(object.getY() - oldY) >= 1;
    ...
    _moveX() {
      ...
      if (this._requestedDeltaX !== 0) {
        ...
        while (this._isCollidingWithOneOf(this._potentialCollidingObjects, floorPlatformId, true)) {
          ...
          if (this._state === this._onFloor) {
            ...
            if (!this._isCollidingWithOneOf(this._potentialCollidingObjects, floorPlatformId, true)) {
              break;
            }
            object.setY(object.getY() + 1);
          }
    ...
    _moveY() {
      ...
      if (this._requestedDeltaY !== 0) {
        ...
        while (this._requestedDeltaY < 0 && this._isCollidingWithOneOf(this._potentialCollidingObjects, null, true) || this._requestedDeltaY > 0 && this._isCollidingWithOneOfExcluding(this._potentialCollidingObjects, this._overlappedJumpThru)) {
    ...
    _checkGrabPlatform() {
      ...
      const collidingPlatforms = gdjs2.staticArray(PlatformerObjectRuntimeBehavior.prototype._checkGrabPlatform);
      ...
      for (const collidingPlatform of collidingPlatforms) {
        ...
        if (!this._isCollidingWithOneOf(this._potentialCollidingObjects, null, true)) {
    ...
    _isCollidingWithOneOf(candidates, exceptThisOne, excludeJumpThrus) {
      excludeJumpThrus = !!excludeJumpThrus;
      for (let i = 0; i < candidates.length; ++i) {
        const platform = candidates[i];
    ...
    _isCollidingWithOneOfExcluding(candidates, exceptTheseOnes) {
      for (let i = 0; i < candidates.length; ++i) {
        const platform = candidates[i];
    ...
    _isCollidingWith(platform) {
      return platform.getPlatformType() !== gdjs2.PlatformRuntimeBehavior.LADDER && !this._isIn(this._overlappedJumpThru, platform.owner.id) && gdjs2.RuntimeObject.collisionTest(this.owner, platform.owner, this._ignoreTouchingEdges);
    }
    ...
    isMoving() {
      return this._hasReallyMoved && (this._currentSpeed !== 0 || this._state === this._onLadder) || this._jumping.getCurrentJumpSpeed() !== 0 || this._currentFallSpeed !== 0;
    }
  }
  ...
  class OnFloor {
    ...
    beforeMovingY(timeDelta, oldX) {
      ...
      if (gdjs2.RuntimeObject.collisionTest(object, this._floorPlatform.owner, behavior._ignoreTouchingEdges)) {
        ...
        while (!behavior._isCollidingWithOneOf(behavior._potentialCollidingObjects)) {
